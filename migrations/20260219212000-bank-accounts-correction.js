...
module.exports = {
  async up(db, client) {
    ...
    const existsNew =
      (await db.listCollections({ name: newName }).toArray()).length > 0;

    if (existsNew) {
      throw new Error(
        `${newName} already exists — aborting migration to avoid data loss`,
      );
    }
  ...
  async down(db, client) {
    ...
    if (existsNew) {
      throw new Error(
        `${newName} already exists — aborting rollback to avoid data loss`,
      );
    }
